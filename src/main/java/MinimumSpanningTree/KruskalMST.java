...
*     是贪心（单一的策略前进）；而动态规划其实每次尝试了拿去所有的物品。
*   - 没有准确的原则能用于判断什么时候应该使用贪心，什么时候用动态规划，什么时候只能回溯（需要培养对算法的深刻理解和在遇
*     到具体问题时敏锐的直觉，这也是算法难的地方）。如果一定要的话，一个“应试”的方法是看数据规模。由于贪心算法的复杂度通
*     常都会很低 O(n) 或 O(nlogn)，而动态规划算法通常都是 O(n^2) 的（但并不一定），所以，如果问题的数据规模是 O(n^2)
*     级别的算法无法承受的话，通常可以想想是否可以贪心。
* */
