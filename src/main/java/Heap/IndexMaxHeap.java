...
*
* - 算法思想：
*   这种给加数据集入反向索引的思路其实是一种经典的算法优化思路，应用十分广泛，另一个典型应用是对排序算法的优化：
*   在之前讲解的排序算法中，排序算法的复杂度主要在于其比较过程。例如，快排的复杂度是 O(nlogn) 是因为其比较操作的次数是 nlogn
*   这个级别的。但是当时我们并没有对交换操作进行优化。如果数据集中每个元素的结构很复杂、挪动成本很高，则算法的整体性能就会大幅
*   很低。此时可以采用反向索引的优化思路 —— 建立一个索引数组，在比较两个元素的时候通过索引取得真正的数据进行比较，但交换的只是
*   索引而非数据集中的元素。
* */
